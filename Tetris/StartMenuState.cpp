...
void StartMenuState::Update(const sf::Time& deltaTime) {
	...
	*m_context->m_backgroundY += backgroundSpeedY * deltaTime.asSeconds();
	...
	m_backgroundSprite.setTextureRect(sf::IntRect(static_cast<int>(*m_context->m_backgroundX),
		static_cast<int>(*m_context->m_backgroundY), m_context->m_window->getSize().x, m_context->m_window->getSize().y));
